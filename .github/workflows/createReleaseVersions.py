...
# erwartete Parameter
parser.add_argument('-m', '--ManifestPath', type=str, help='The Path where all manifest:_all.json are located', default='manifests')
...
if args.ManifestPath:
    ...
    extracted_data = search_manifests_and_extract_version(args.ManifestPath)

    # Wenn Daten extrahiert wurden, speichere sie in der JSON-Datei 'versions.json'
    if extracted_data:
        save_results_to_json(extracted_data, os.path.join(args.VersionDir, 'releaseversions.json'))
    else:
